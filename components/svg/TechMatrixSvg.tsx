...
export function TechMatrixSvg() {
  ...
  return (
    <div className="w-full my-8 bg-[#FFFEFA] border border-[rgba(10,39,71,0.14)] rounded-xl p-5 md:p-8 shadow-sm">
      ...
      {/* Mobile Card-Based Architecture View (STRICTLY MOBILE-FRIENDLY & NO OVERLAPS) */}
      <div className="block md:hidden space-y-4">
        {matrixProjects.map((proj) => (
          <div
            key={proj.name}
            className="p-4 rounded-lg bg-[#F6F3EC] border border-[rgba(10,39,71,0.12)] space-y-3"
          >
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {proj.techs.map((tech) => (
                <span
                  key={tech}
                  className="font-mono text-xs px-2.5 py-1 rounded bg-[#FFFEFA] text-[#0A2747] border border-[rgba(10,39,71,0.12)] font-medium"
                >
                  {tech}
                </span>
              ))}
